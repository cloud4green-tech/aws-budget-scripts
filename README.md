...
---
25. Important Notes
DynamoDB budget rows confirm budget assignment.
StackSet baseline is required for full event forwarding and enforcement.
Direct Bedrock Console usage may not give perfect real-time token costing.
Bedrock proxy is required for the most accurate real-time token reserve/refund flow.
AWS model token limits and service quotas still apply.
Explicit deny always overrides AdministratorAccess.
After IAM Identity Center permission set changes, students must sign out and sign in again.
---
26. License
Internal use only.
